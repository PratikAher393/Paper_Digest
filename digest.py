...
def save_digest(by_topic, total):
    """Save the digest as JSON to docs/ for the GitHub Pages site."""
    docs = HERE / "docs" / "digests"
    docs.mkdir(parents=True, exist_ok=True)
    
    # Filename: digests/2026-09-11.json
    filename = docs / f"{TODAY}.json"
    data = {
        "date": str(TODAY),
        ...
        "topics": {topic: [
            {"title": p["title"], "authors": p["authors"], "abstract": p["abstract"],
             "source": p["source"], "url": p["url"], "score": p["score"]}
            for p in papers
        ] for topic, papers in by_topic.items()}
    }
    filename.write_text(json.dumps(data, indent=2), encoding="utf-8")
    
    # Update archive index: docs/digests.json lists all past digests
    index_file = HERE / "docs" / "digests.json"
    if index_file.exists():
        index = json.loads(index_file.read_text(encoding="utf-8"))
    else:
        index = []
    
    # Add this digest if not already there
    if not any(d["date"] == str(TODAY) for d in index):
        index.append({"date": str(TODAY), "count": total})
